...
$(document).ready(function() {
  ...
  const calculateTweetAge = function(tweetDate) {
    let today = new Date();
    let tweetPost = new Date(tweetDate)
    let diffInMs = today - tweetPost;
    let diffInDays = diffInMs / (1000 * 60 * 60 * 24);
    let tweetAgeDays = Math.floor(diffInDays);
    let tweetToday = "";
    if (tweetAgeDays === 0) {
      tweetToday = tweetPost.getMonth()+'/'+tweetPost.getDate()+'/'+tweetPost.getFullYear();
    } else if (tweetAgeDays === 1) {
      tweetToday = '1 day ago';
    } else if (tweetAgeDays > 1) {
      tweetToday = `${tweetAgeDays} days ago`;
    }
    return tweetToday
  }  
  ...
  const createTweetElement = function(data) {
    ...
    const date = calculateTweetAge(data.created_at)
